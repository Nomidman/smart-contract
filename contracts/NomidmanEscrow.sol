...
contract NomidmanEscrow {
    ...
    enum GasSpendingAction {
        RELEASE,
        DISABLE_SELLER_CANCEL,
        BUYER_CANCEL,
        SELLER_CANCEL,
    ...
    constructor() public {
        ...
        gasEstimations[keccak256(
            abi.encodePacked(GasSpendingAction.RELEASE)
        )] = 36100;
        gasEstimations[keccak256(
            abi.encodePacked(GasSpendingAction.DISABLE_SELLER_CANCEL)
        )] = 12100;
        gasEstimations[keccak256(
            abi.encodePacked(GasSpendingAction.BUYER_CANCEL)
        )] = 36100;
        gasEstimations[keccak256(
            abi.encodePacked(GasSpendingAction.SELLER_CANCEL)
        )] = 36100;
        gasEstimations[keccak256(
            abi.encodePacked(GasSpendingAction.SELLER_CANCEL_REQUEST)
        )] = 12100;
        gasEstimations[keccak256(
            abi.encodePacked(GasSpendingAction.RESOLVE_DISPUTE)
        )] = 36100;
        gasEstimations[keccak256(
            abi.encodePacked(GasSpendingAction.BATCH_RELAY)
        )] = 28500;
    }
    ...
    function createEscrow(
        ...
        bytes32 combinedhash = keccak256(
            abi.encodePacked(_tradeHash, _paymentWindowInSeconds)
        );
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 prefixedHash = keccak256(
            abi.encodePacked(prefix, combinedhash)
        );

        require(!escrows[_tradeHash].exists);
        //require(ecrecover(prefixedHash, _v, _r, _s) == relayer);
        // require(msg.sender == relayer); // TODO: CHANGE THIS
        //require(block.timestamp < _expiry);
        require(msg.value > 0 && msg.value == _value);
        uint32 canBeCancelledBySellerWithin = _paymentWindowInSeconds == 0
            ? 1
    ...
    function createEscrowTest(
        ...
        escrows[_tradeHash] = Escrow(true, 360, 180);
    }

    function withdrawFees(uint256 _amount, address payable _receiver)
        public
        onlyManager
    {
    ...
    function doRelease(
        ...
        bytes32 _tradeHash = getTradeHash(
            _tradeID,
            _seller,
            _buyer,
            _value,
        ...
        uint128 _gasFees = uint128(
            _escrow.relayerGasBalance.add(
                msg.sender == relayer
                    ? gasEstimations[keccak256(
                        abi.encodePacked(GasSpendingAction.RELEASE)
                    )]
                        .add(_additionalGas)
                        .mul(uint128(tx.gasprice))
                    : 0
            )
        );
    ...
    function doSellerCancel(
        ...
        if (
            _escrow.canBeCancelledBySellerWithin <= 1 ||
            _escrow.canBeCancelledBySellerWithin > block.timestamp
        ) {
            return false;
        }
    ...
    function doSellerRequestCancel(
        ...
        escrows[_tradeHash].canBeCancelledBySellerWithin =
    ...
    function resolveDispute(
        ...
        address _signature = ecrecover(
            keccak256(abi.encodePacked(_tradeID, DISPUTE_CODE)),
            _v,
            _r,
            _s
        );
        ...
        uint256 _totalFees = _escrow.relayerGasBalance +
            gasEstimations[keccak256(
                abi.encodePacked(GasSpendingAction.RESOLVE_DISPUTE)
            )];
    ...
    // ADDED FUNCTION
    function resolveDisputeUpdated(
        bytes16 _tradeID,
        address _seller,
        address _buyer,
        address payable _inFavor,
        uint256 _value,
        uint256 _fee
    ) external onlyMediator {
        bytes32 _tradeHash = getTradeHash(
    ...
    function setArbitrator(address _newMediator) external onlyManager {
    ...
    function setRelayer(address _newRelayer) external onlyManager {
        relayer = _newRelayer;
    }
}
